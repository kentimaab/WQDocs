...
| Problem | Solution |
|---|---|
| Entries do not appear after saving | Check that `loggWildCardReload` is set to `true` in DataStore. When `true`, the handler reloads all entries under the topic using a wildcard match after each save, so newly created sub-topic entries appear immediately. When `false`, only exact-topic entries reload. |
